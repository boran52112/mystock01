...
def generate_expert_debate_db3(db2, has_chip):
    ...
    if has_chip:
        ...
        if inst_net > 500: bull_score += 3; table_data.append(["8. 法人籌碼", "🟢 看多", f"買超 {int(inst_net)}張"])
        elif inst_net < -500: bear_score += 3; table_data.append(["8. 法人籌碼", "🔴 看空", f"賣超 {int(inst_net)}張"])
        else: table_data.append(["8. 法人籌碼", "⚪ 中立", "觀望"])
    else: table_data.append(["8. 法人籌碼", "⚪ 未知", "無資料"])

    # 9. 散戶 (W2)
    if has_chip:
        if latest['Margin_Diff'] > 500 and latest['Close'] < prev['Close']:
            bear_score += 2; table_data.append(["9. 散戶籌碼", "🔴 看空", "資增價跌"])
        else: table_data.append(["9. 散戶籌碼", "⚪ 中立", "穩定"])
    else: table_data.append(["9. 散戶籌碼", "⚪ 未知", "無資料"])

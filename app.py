...
# 【修改1】輸入框只要求輸入數字
stock_code = st.sidebar.text_input("請輸入台股代號 (例如: 2330 或 8069)", "2330")
update_button = st.sidebar.button("🔄 每日更新資料")

# DB1：原始資料庫 (加入自動判斷上市/上櫃功能)
def fetch_stock_data(code):
    try:
        ...
        actual_symbol = symbol_tw
        
        # 如果上市找不到資料 (表格是空的)，就改用「上櫃 (.TWO)」抓取
        if df.empty:
            symbol_two = f"{code}.TWO"
            ticker = yf.Ticker(symbol_two)
            df = ticker.history(period="1y")
            actual_symbol = symbol_two
...
if update_button or stock_code:
    ...
    if db1_price_data is not None:
        ...
        st.write(f"### 🎯 目前分析標的：{stock_code} ({market_type})")
        ...
        st.error(f"❌ 找不到代號為「{stock_code}」的股票，請確認是否為有效的台股代號。")
